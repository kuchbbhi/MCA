...
def q_learning():
    ...
    for episode in range(N_EPISODES):
        ...
        while not is_terminal(state):
            # 1. Choose action A using epsilon-greedy
            ...
            # Off-policy: use the max Q-value of the next state regardless of the action taken there
            max_next_q = np.max(Q[next_state, :])
            
            # Q(S', A') is 0 if S' is terminal
            Q_next = 0.0 if is_terminal(next_state) else max_next_q
...
print("🏁 Optimal Q-Table Learned by Q-Learning (Max Q-value per state):\n")
