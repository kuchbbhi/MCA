...
def sarsa_learning():
    ...
    for episode in range(N_EPISODES):
        ...
        # 1. Choose initial action A using epsilon-greedy
        ...
        while not is_terminal(state):
            ...
            # 4. Choose next action A' using epsilon-greedy (On-Policy step!)
            next_action = choose_action(next_state, Q, current_epsilon)
            
            # --- SARSA Update Rule ---
            
            # If next_state is terminal, Q(S', A') is 0, otherwise it's the learned value
            Q_next = 0.0 if is_terminal(next_state) else Q[next_state, next_action]
...
def extract_optimal_policy(Q):
    ...
    for state in range(N_STATES):
        if is_terminal(state):
            continue
        # The optimal policy is extracted greedily from the final Q-table
        policy[state] = np.argmax(Q[state, :])
    return policy


# --- 5. Execution ---
optimal_Q = sarsa_learning()
optimal_policy = extract_optimal_policy(optimal_Q)

print("🏁 Optimal Q-Table Learned by SARSA (Max Q-value per state):\n")
