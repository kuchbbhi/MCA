...
class PolicyNetwork(nn.Module):
    ...
    def forward(self, x):
        # Apply Softmax to output probabilities for actions
...
# ------------------------- 
# 3. REINFORCE with Baseline Algorithm
# ------------------------- 
...
def reinforce_with_baseline(env_name="CartPole-v1", episodes=1000, gamma=0.99):
    ...
    for ep in range(episodes):
        ...
        while True:
            ...
            # Store the current state, action, and log probability
        ...
        returns_t = calculate_returns(rewards, gamma)
        
        # 2. Update Value Network (Baseline)
        
        # Predict V(s) for all states in the episode
        predicted_values = value_net(states_t).squeeze(-1)
        
        # Value Loss (Mean Squared Error between G_t and V(s))
        ...
        # Calculate Advantage: A_t = G_t - V(s)
        # Use the predicted values from the (now updated) Value Network as the baseline
        # The .detach() ensures no gradient flows back to the value network weights.
        advantage = returns_t - predicted_values.detach()
        # 
        
        # Policy Loss: - log(pi(A|S)) * Advantage
        # We multiply by -1 because optimizers minimize loss, but we want to maximize the return
        ...
        if ep % 50 == 0:
            print(f"Episode {ep:4d}, Reward: {total_reward:6.1f}, Value Loss: {value_loss.item():.4f}")
            
        # Stopping condition for CartPole (average reward over last 100 episodes >= 475)
...
if __name__ == '__main__':
    ...
    print("Starting REINFORCE with Baseline training on CartPole-v1...")
    
    rewards = reinforce_with_baseline() 
    ...
    plt.title("REINFORCE with Baseline Training Performance") 
    plt.grid(True)
    plt.show()